...
function createPage(el) {
    ...
    // 添加图标
    $(el + ' .prev_top_btn').append('<span class="iconfont_ icon-most-left"></span>')
    // 创建上一步按钮
    element.append('<a class="prev_btn page_common_btn_style"></a>')
    // 添加图标
    $(el + ' .prev_btn').append('<span class="iconfont_ icon-zuo"></span>')
    // 创建页码按钮框
    element.append('<div class="page_btn_box"></div>')
    // 创建下一步按钮
    element.append('<a class="next_btn page_common_btn_style"></a>')
    // 添加图标
    $(el + ' .next_btn').append('<span class="iconfont_ icon-you"></span>')
    // 创建跳到最后一页按钮
    element.append('<a class="next_bottom_btn page_common_btn_style"></a>')
    // 添加图标
    $(el + ' .next_bottom_btn').append('<span class="iconfont_ icon-most-right"></span>')
    // 返回 el
    return {
        el: el, //选择器
...
function setPage(dom, pageData) {
    ...
    function createBtnItem(pageCurrent){
        ...
        for(var i=minPageNum;i<maxPageNum+1;i++){
            if(i==pageCurrent){ 
                html_S='<a class="page_common_btn_style page_common_btn_active" data-pageIndex="'+i+'">'+i+'</a>';
            }else{ 
                html_S='<a class="page_common_btn_style" data-pageIndex="'+i+'">'+i+'</a>';
            }
            if(i==minPageNum&&minPageNum!=1){
                html_S='<a class="page_common_btn_style">···</a>'
            }
